...
- a megajándékozandó személyek alapvető adatait listában (BindingList) tároljuk (nem használunk mögöttes adatbázist)
- a bevitt személyes adatok helyességét ellenőrizzük (Regex - Validate metódus) - Unit Testtel ellenőrizzük, hogy biztos megfelelő legyen a validációs és regisztrációs metódus (összesen 4 teszt)
...
IRecipientManager:
- interfész
- ez alatt Recipienthez BindingList + CreateRecipient metódus (még nincs kifejtve)

B) Entities mappa (az interfész és a Form1 kivételével minden osztály ebben van)
a)
RecipientManager:
- az őse az IRecipientManager interfész
- interfészhez illeszkedve Recipients lista + CreatRecipient függvény kifejtve
- azonos iTunes fiókkal csak 1 megajándékozandó személy regisztrálható
b)
RecipientController:
- tárolja az RecipientManager egy példányát
...
C) Form1:
- BindingList-ek és RecipientController példányosítása
...
- Hozzáadás... gomb= ButtonAddList_Click metódusa elvégzi a bevitt, megajándékozott személyre vonatkozó adatok regisztrációját, ha a regisztráció sikeres, a beviteli mezőket törli, ha nem, a validációs metódusoknak megfelelően hibaüzenetet ad vissza, de a téves adatok a TextBox-ban maradnak, hogy ne kelljen minden újra begépelni
...
- a RecipientControllerhez megírt Unit Testeket tartalmazza
