        ...
        public Account Register(string fullname, string shortname, string email)
        {
            if (!ValidateFullName(fullname))
                throw new ValidationException(
                    "A megadott teljes név nem megfelelő! Kérlek, ne használj ékezeteket! A keresztnév és a vezetéknév is nagy betűvel kezdődjön!");

            if (!ValidateShortName(shortname))
                throw new ValidationException(
                    "A megadott becenév nem megfelelő! Kérlek, ne használj ékezeteket! A becenév egyetlen rövid szóból állhat!");
